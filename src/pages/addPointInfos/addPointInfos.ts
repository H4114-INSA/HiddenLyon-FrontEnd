...
export class AddPointInfosPage {
  ...
  constructor(g:Globals,serv:POIService, r: Router){
  this.tempo=false;
  this.globals=g;
  ...
  addPoint(){
    ...
    if(!this.tempo){
      ...
      let poi=new PointOfInterest(this.titre,this.descr,this.globals.photoTaken,user,this.catSelect,this.globals.photoTakenLon,this.globals.photoTakenLat);
      this.poiService.addPoint("http://localhost:8080/poi/add",poi,this.globals.userExtended.token);
    }
    else{
      let poi=new TemporaryPointOfInterest(this.titre,this.descr,this.globals.photoTaken,user,this.catSelect,this.globals.photoTakenLon,this.globals.photoTakenLat,this.date);
